...
router.route('/folder')
    .post(function (req, res) {
        ...
        folder.save(function (err) {
            ...
            res.json({ code: 0, msg: 'folder created!' });
        });
    })
...
router.route('/folder/:id')
    .put(function (req, res) {
        Folder.find({ _id: req.params.id }, function (err, folder) {
            if (err)
                res.json({ code: 1, msg: 'find doc err', err: err });
            folder.foldername = req.body.foldername;
            folder.save(function (err) {
                if (err)
                    res.json({ code: 2, msg: 'doc save err', err: err });
                res.json({ code: 0, msg: 'Folder updated!' });
            });
        });
    })
    ...
    .delete(function (req, res) {
        Folder.findById(req.params.id, function (err, folder) {
            ...
            if (err)
                ...
                res.json({ code: 2, msg: 'folder not exist' });
            else if (folder.username !== req.session.username)
                res.json({ code: 3, msg: 'not your folder', err: err });
            else {
                folder.remove();
                res.json({ code: 0, msg: 'Successfully deleted' });
            }
        });
    });
...
router.route('/folder/:type/:folderId')
    .get(function (req, res) {
        if (req.params.type === 'diary')
            Diary.find({ folderId: req.params.folderId }, function (err, diary) {
                if (err)
                    res.json({ code: 1, msg: 'find doc err', err: err });
                res.json({ code: 0, data: diary });
            });
        else if (req.params.type === 'phonebook')
            Phonebook.find({ folderId: req.params.folderId }, function (err, phonebook) {
                if (err)
                    res.json({ code: 1, msg: 'find doc err', err: err });
                res.json({ code: 0, data: phonebook });
            });
        else if (req.params.type === 'todolist')
            Todolist.find({ folderId: req.params.folderId }, function (err, todolist) {
                if (err)
                    res.json({ code: 1, msg: 'find doc err', err: err });
                res.json({ code: 0, data: todolist });
            });
        else {
            res.json({ code: 9, msg: 'wrong type' });
        }
    })
